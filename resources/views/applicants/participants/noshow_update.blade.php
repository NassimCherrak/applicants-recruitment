<div class="col-md-8 order-md-1">
	...
	<form class="need-validation" method="POST" action="<?= $myPath ?>/upnoshow/{{ $selectedParticipant->id_contact }}">
		{{ csrf_field() }}
		<div class="row">
		...
		<div class="form-check">
			<label>
				<input type="radio" name="status" value="No Show" onclick="selectNoShowOff();" checked><span>Keep current status</span>
			</label>
		</div>
		<div class="form-check">
			<label>
				<input type="radio" name="status" value="New" onclick="selectNoShowOn();"><span>Schedule new appointment</span>
			</label>
		</div>
		<div class="mb-3" id="noshow-option-1">
			<label class="control-label" for="date">New Appointment Date:
			</label>
